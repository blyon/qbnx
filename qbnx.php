...
// Run QBNX with Args.
for ($cnt = 1; $cnt <= RETRY_COUNT; $cnt++) {
    if (true === runQbnx($args)) {
        break;
    }
    $log = Log::getInstance();
    $log->write(Log::INFO, "Retry Attempt #" . $cnt);
}
 ...
 *
 * @return boolean False if an exception is caught.
 */
function runQbnx($args) {
    try {
